...
async function createBooking(req, res) {
    ...
    try {
        ...
        if(area.max_hours_per_day_enabled) {
            ...
            let [dayMinutes] = await Model.readBookingMinutesPerInterval(req.params.system, req.body.create_by, intervalCurrentDayStart, intervalCurrentDayEnd)
            const currentDayMinutes = dayMinutes?.summa || 0;
            const maxMinutesDay = area.max_hours_per_day * 60;
            //Den aktuella bokningens längd i minuter
            const bookingDuration = (req.body.end_time - req.body.start_time) / 60;
        ...
        if (req.params.librarycode == process.env.MAIN_LIBRARY_CODE) {
            if (!libraryclosed) {
                if (ismanned) {
                    ...
                    if (moreopen) {
                        opentodaymoretext_start = `${openinfotext_1_startpage} ${openingmorehoursarr[0].replace(/[.:]00/g, '')}–${openingmorehoursarr[1].replace(/[.:]00/g, '')} ${openinfotext_2_startpage}`;
                    } else {
                        opentodaymoretext_start = ""
                    }
                    opentodayhours = `${firsthour}–${lasthour}${moreopen ? '*' : ''}`;
                }
            } else {
                opentodaymoretext_start = ""
