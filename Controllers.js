    ...
    try {
        ...
        if(area.max_hours_per_day_enabled) {
            ...
            if (dayHours.summa + bookingDuration > maxHoursDay) {
                return { status: 1, violation: true, policytype: "Max hours per day", bookedhours: dayHours, message: `You have already booked ${dayHours.summa} today. The maximum number of hours per day per user is ${maxHoursDay}` }; 
            }
        }
        ...
        if(area.max_hours_per_week_enabled) {
            ...
            if (weekHours.summa + bookingDuration > maxHoursWeek) {
                return { status: 1, violation: true, policytype: "Max hours per week", bookedhours: weekHours, message: `You have already booked ${weekHours.summa} this week. The maximum number of hours per week per user is ${maxHoursWeek}`}; 
            }
        }
